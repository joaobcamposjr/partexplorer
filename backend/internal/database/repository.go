...
type PartRepository interface {
	...
	CleanDuplicateNames() (map[string]interface{}, error)
...
func (r *partRepository) GetPartByID(id string) (*models.SearchResult, error) {
	...
	// Carregar estoques manualmente - SIMPLES E SEM DUPLICATAS
	var allStocks []models.Stock
...
// SearchPartsByBrand busca peças por marca
func (r *partRepository) SearchPartsByBrand(brandName string, page, pageSize int, availableOnly bool, includeObsolete bool) (*models.SearchResponse, error) {
	...
	// Buscar part_groups que têm peças da marca especificada
	var partGroups []models.PartGroup
	...
	// Carregar dados completos dos part_groups
	for i := range partGroups {
		if err := r.db.Preload("ProductType").
			Preload("ProductType.Subfamily").
			Preload("ProductType.Subfamily.Family").
			Preload("Dimension").
			First(&partGroups[i], partGroups[i].ID).Error; err != nil {
			return nil, fmt.Errorf("failed to load part group data: %w", err)
		}
	}
