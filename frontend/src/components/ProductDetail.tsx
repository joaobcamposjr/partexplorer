...
const ProductDetail: React.FC<ProductDetailProps> = ({ productId, onBackToResults }) => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(config.getApiUrl(`/api/v1/search?q=${encodeURIComponent(searchQuery)}`));
      if (response.ok) {
        const data = await response.json();
        console.log('Resultados da busca:', data);
        
        // Navegar para a página de resultados
        window.location.href = config.getAppUrl(`?q=${encodeURIComponent(searchQuery)}`);
      }
    } catch (error) {
  ...
  const handleSuggestionClick = async (suggestion: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Navegar para a página de resultados
        window.location.href = config.getAppUrl(`?q=${encodeURIComponent(suggestion)}`);
      }
    } catch (error) {
  ...
  const fetchProductDetail = async () => {
    try {
      ...
      let url;
      if (isUUID) {
              url = config.getApiUrl(`/api/v1/parts/${productId}`);
    } else {
      url = config.getApiUrl(`/api/v1/parts/sku/${productId}`);
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            {/* Ficha Técnica - MOVIDA PARA BAIXO DA IMAGEM */}
            <div className="mt-6">
          ...
          {/* Right Panel - Estoques Disponíveis */}
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              {/* DEBUG: Verificar dados de estoque */}
              {(() => {
              ...
              {product.stocks && product.stocks.length > 0 ? (
                ...
                <div className="text-center py-8">
                  <p className="text-gray-500">Nenhum estoque disponível encontrado</p>
                  <p className="text-sm text-gray-400 mt-2">Verifique se há empresas com este produto em estoque</p>
                </div>
              )}
            </div>
        ...
        <div className="space-y-8">
          {/* Companies with Stock - REMOVIDO (duplicado) */}
